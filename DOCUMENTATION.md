...
| `used_a_dash(dashes_left : int, dashes_used : int, max_dashes : int)` |  emitted when the character performs a dash. Returns how many dashes the character has left and what the maximum amount of dashes are.  |
...
| `knocked_back(direction : Vector2 , strength : float)` |  emitted when the character is being knocked back. It returns the direction and the strength of the knockback as seperate values.  |
